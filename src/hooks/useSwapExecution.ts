...
export const useSwapExecution = (): UseSwapExecutionReturn => {
  ...
  // CRITICAL FIX: Enhanced approval with better error handling and retry logic
  const approveToken = useCallback(async (
    ...
    try {
      ...
      // CRITICAL FIX: Increased gas limit for approval transactions
      const txHash = await provider.request({
        ...
        params: [{
          ...
          to: tokenAddress,
          data: data,
          gas: '0x1C9C380', // 30,000,000 gas limit (increased)
        }]
      });
  ...
  const waitForTransaction = useCallback(async (txHash: string, maxAttempts: number = 30): Promise<boolean> => {
    try {
      const { provider } = await getProviderAndSigner();
      if (!provider) throw new Error('No provider available');
      ...
      for (let i = 0; i < maxAttempts; i++) {
        try {
          ...
          console.log(`⏳ Waiting for transaction... (${i + 1}/${maxAttempts})`);
        }
    ...
    try {
        ...
        if (currentAllowanceBig < requiredAllowance) {
          ...
          try {
            if (currentAllowanceBig > BigInt(0)) {
              ...
              await approveToken(params.tokenIn.address, SPHYNX_CONTRACTS.ROUTER, '0', params.tokenIn.decimals, 1.0);
              await new Promise(resolve => setTimeout(resolve, 3000));
            }
          } catch (error) {
            console.warn('⚠️ Could not reset allowance, proceeding with direct approval');
          }
          ...
          const approvalConfirmed = await waitForTransaction(approvalTx, 30);
      ...
      const txHash = await provider.request({
        ...
        params: [{
          ...
          gas: '0x7A120', // 500,000 gas limit (increased)
        }]
      });
      ...
      // CRITICAL FIX: Enhanced error analysis to prevent false "insufficient balance" errors
      let errorMessage = error.message || 'Sphynx swap failed';
      
      // Analyze the actual error to provide accurate feedback
      if (error.message.includes('transferFrom failed') || error.message.includes('TRANSFER_FROM_FAILED')) {
        errorMessage = 'Token transfer failed. This may be due to insufficient allowance or a token contract issue.';
        ...
        // Generic revert - try to provide more context
        if (error.message.includes('balance')) {
          errorMessage = error.message; // Use the specific balance error
        } else {
        ...
        // Only show insufficient balance if we're confident it's accurate
  ...
  const executeSwapOnReachSwap = useCallback(async (params: SwapParams): Promise<SwapResult> => {
    try {
      ...
      console.log(`🔄 Mock ReachSwap swap: ${params.amountIn} ${params.tokenIn.symbol} → ${params.tokenOut.symbol}`);
      ...
      return {
        success: false,
        error: error.message || 'ReachSwap swap failed',
        routerUsed: 'reachswap'
      };
    }
  }, [getRouterForPair]);
