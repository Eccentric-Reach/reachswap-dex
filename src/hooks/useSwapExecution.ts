...
export const useSwapExecution = (): UseSwapExecutionReturn => {
  ...
  const waitForTransaction = useCallback(async (txHash: string, maxAttempts: number = 60): Promise<boolean> => {
    try {
      const { provider } = await getProviderAndSigner();
      if (!provider) throw new Error('No provider available');
        ...
        try {
          ...
          if (receipt) {
            if (receipt.status === '0x1') {
              ...
              console.error(`❌ Transaction failed: ${txHash}, status: ${receipt.status}`);
              return false;
            }
          }
        } catch (error) {
      ...
      // Don't fail on timeout - transaction might still succeed
